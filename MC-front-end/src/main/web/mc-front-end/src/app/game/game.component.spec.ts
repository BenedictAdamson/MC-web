...
describe('GameComponent', () => {
  ...
  const canCreate = (self: User, identifier: GameIdentifier, runState: string,
    ...
    const game: Game = new Game(identifier, runState, true, users);
    setUp(self, game);
    tick();
    fixture.detectChanges();
  ...
  const testStartGame = (identifier: GameIdentifier) => {
    ...
    const game0: Game = new Game(identifier, runState0, true, users);
    setUp(self, game0);
  ...
  const testStopGame = (identifier: GameIdentifier) => {
    const self: User = USER_ADMIN;
