...
export abstract class AbstractSelfService {
   ...
   private hasRole2$(role1: string, role2: string): Observable<boolean> {
      return this.authorities$.pipe(
         map(
            authorities => authorities.includes(role1) || authorities.includes(role2)
         )
      );
   }
   ...
   get mayListUsers$(): Observable<boolean> {
      return this.hasRole2$('ROLE_PLAYER', 'ROLE_MANAGE_USERS');
   }

   get mayExamineGame$(): Observable<boolean> {
      return this.hasRole2$('ROLE_PLAYER', 'ROLE_MANAGE_GAMES');
   }
